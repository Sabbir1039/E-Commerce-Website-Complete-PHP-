                ...
                if($rowcount!=0){
                  while($row=mysqli_fetch_assoc($data)){
                    echo "
                      ...
                      <td>
                        <a style='color: DarkCyan; font-size: 25px;' href='product_update.php?id=$row[id]&name=$row[name]&category=$row[category]&sub_category=$row[sub_category]&color=$row[color]&quantity=$row[quantity]&price=$row[price]'>
                        <i class='fas fa-edit'></i></a>
                      </td>
                      <td>
                        <a href='product_delete.php?id=$row[id]' style='color: red; font-size: 25px;'><i class='fas fa-trash-alt'></i></a>
                      </td></tr>";
                  }
                }
              }else {
